...
void planif_encolar_NEW(t_pedido* pedido) {
	...
	log_info(logger_obligatorio, "(Ped. %i - Rest. %s) Cambio de cola: [*] => [NEW] (Es un nuevo pedido)",
		pedido->pedido_id,
...
void* fhilo_planificador_largoPlazo(void* __sin_uso__) { // (de NEW a READY)
	...
	while (1) {
		...
		log_info(logger_obligatorio, "(Ped. %i - Rest. %s - Rep. %i) Se asigno el pedido al repartidor (rep. en (%i, %i), rest. en (%i, %i), cliente en (%i, %i))",
			...
			pedido_seleccionado->cliente->pos_y
		);
...
t_pedido* planif_FIFO(void) {
	t_pedido* pedido = list_remove(cola_READY, 0);
	log_debug(logger_planificacion, "[PLANIF_CP] Desencolando pedido de READY...");
	log_info(logger_obligatorio, "(Ped. %i - Rest. %s - Rep. %i) Cambio de cola: [READY] => [EXEC] (Elegido por el PCP por primero en cola)",
		pedido->pedido_id,
		pedido->restaurante->nombre,
...
t_pedido* planif_SJF_SD(void) {
	...
	pedido = list_remove(cola_READY, index_pedido_min);
	log_info(logger_obligatorio, "(Ped. %i - Rest. %s - Rep. %i) Cambio de cola: [READY] => [EXEC] (Elegido por el PCP por minima estimacion de rafaga (%.4f))",
		pedido->pedido_id,
		pedido->restaurante->nombre,
		pedido->repartidor->id,
		estimacion_min
...
double estimar_rafaga(t_pedido* pedido) {
	...
	return ((double)(pedido->sjf_ultRafaga_real)) * cfval_alpha
			+ (pedido->sjf_ultRafaga_est) * (1.0 - cfval_alpha);
}

t_pedido* planif_HRRN(void) {
	t_pedido* pedido;
	unsigned index_pedido_max = 0;
	double respRatio_max = respRatio(list_get(cola_READY, index_pedido_max));
	for (
			unsigned index_READY = 1;
			index_READY < list_size(cola_READY);
			index_READY++
	...
	pedido = list_remove(cola_READY, index_pedido_max);
	log_info(logger_obligatorio, "(Ped. %i - Rest. %s - Rep. %i) Cambio de cola: [READY] => [EXEC] (Elegido por el PCP por maximo response ratio (%.4f))",
		pedido->pedido_id,
		pedido->restaurante->nombre,
		pedido->repartidor->id,
		respRatio_max
...
double respRatio(t_pedido* pedido) {
	return 1.0 + (double)(pedido->hrrn_tiempoEsperaREADY) / estimar_rafaga(pedido);
}
...
void* fhilo_pedido(void* pedido_sin_castear) { // toma t_pedido* por param
	...
	while (
		...
		log_info(logger_obligatorio, "(Ped. %i - Rest. %s - Rep. %i) El repartidor se movio a (%i, %i)",
			pedido->pedido_id,
			pedido->restaurante->nombre,
			pedido->repartidor->id,
...
void pedido_repartidorLlegoARestaurante(t_pedido* pedido) {
	...
	if (pedido->restaurante != resto_default) {
		...
		if (noEstaPreparado) {
			...
			log_info(logger_obligatorio, "(Ped. %i - Rest. %s - Rep. %i) Cambio de cola: [EXEC] => [BLOCK] (El repartidor llego al restaurante pero no todos los platos estan preparados todavia)",
				pedido->pedido_id,
				pedido->restaurante->nombre,
				pedido->repartidor->id
			);
			...
			log_info(logger_obligatorio, "(Ped. %i - Rest. %s - Rep. %i) Cambio de cola: [BLOCK] => [READY] (Ya se prepararon todos los platos del pedido)",
				pedido->pedido_id,
				pedido->restaurante->nombre,
				pedido->repartidor->id
			);
...
void pedido_repartidorLlegoACliente(t_pedido* pedido) {
	...
	t_cliente* cliente = pedido->cliente;

	pthread_mutex_lock(&mutex_pedidosEXEC);
	// pthread_mutex_lock(&mutex_pedidosEXIT);
	search_remove_return(pedidosEXEC, pedido);
	pedido->pedido_estado = EXIT;
	log_info(logger_obligatorio, "(Ped. %i - Rest. %s - Rep. %i) Cambio de cola: [EXEC] => [EXIT] (El repartidor llego al cliente)",
		pedido->pedido_id,
		pedido->restaurante->nombre,
		pedido->repartidor->id
	);
	...
	enviar_mensaje(mje_env_cli, cliente->socket);
	pthread_mutex_unlock(cliente->mutex);
	log_info(logger_obligatorio, "(Ped. %i - Rest. %s - Rep. %i) Se entrego el pedido al cliente %i",
		pedido->pedido_id,
		pedido->restaurante->nombre,
		pedido->repartidor->id,
		cliente->id
...
void consumir_ciclo(t_pedido* pedido) {
	...
	log_info(logger_obligatorio, "(Ped. %i - Rest. %s - Rep. %i) Cambio de cola: [EXEC] => [BLOCK] (El repartidor debe descansar)",
		pedido->pedido_id,
		pedido->restaurante->nombre,
		pedido->repartidor->id
	);
	...
	log_info(logger_obligatorio, "(Ped. %i - Rest. %s - Rep. %i) Cambio de cola: [BLOCK] => [READY] (El repartidor finalizo su descanso)",
		pedido->pedido_id,
		pedido->restaurante->nombre,
		pedido->repartidor->id
	);
...
void destroy_pedido(t_pedido* pedido) {
	pthread_mutex_destroy(pedido->estaPreparado);
	pthread_mutex_destroy(pedido->mutex_clock);
	pthread_mutex_destroy(pedido->mutex_EXEC);
}
...
void process_request(int cod_op, int cliente_fd) {
	...
	buffer = recibir_mensaje(cliente_fd, &error);
	...
	case AGREGAR_PLATO:
		...
		// mensaje liberado dentro de la funcion
		break;
...
bool esPlatoRestoDefault(char* nombre_plato) {
	unsigned index = 0;
	while (cfval_platosDefault[index]) {
		if (string_equals_ignore_case(
			nombre_plato,
			cfval_platosDefault[index++]
		)) {
			return true;
		}
	}
...
void gestionar_AGREGAR_PLATO(t_nombre_y_id* plato, int cliente_id, int socket_cliente) {
	...
	if (!cliente_en_cuestion) {
		...
		free_struct_mensaje(plato, AGREGAR_PLATO);
		return;
	}
	pthread_mutex_lock(cliente_en_cuestion->mutex);
	...
	if (resto_en_cuestion == resto_default) {
		if (!esPlatoRestoDefault(plato->nombre.nombre)) {
			log_debug(logger_mensajes, "[MENSJS]: %s no es un plato del RestoDefault -> FAIL a cli", plato->nombre.nombre);
			responder_confirm(socket_cliente, false, RTA_AGREGAR_PLATO);
			free_struct_mensaje(plato, AGREGAR_PLATO);
			pthread_mutex_unlock(cliente_en_cuestion->mutex);
			return;
...
void gestionar_CONSULTAR_PEDIDO(uint32_t* id_pedido, int socket_cliente, int cliente_id) {
	...
	cliente_en_cuestion = get_cliente_porSuID(cliente_id);
    if (!cliente_en_cuestion) {
        log_debug(logger_mensajes, "[MENSJS] El cliente %i no esta registrado -> ERROR",
            cliente_id
        );
        responder_ERROR(socket_cliente);
		free(id_pedido);
    ...
    if (!rest_en_cuestion) {
        log_debug(logger_mensajes, "[MENSJS] El cliente %i no selecciono rest. -> ERROR",
            cliente_id
        );
        responder_ERROR(socket_cliente);
		free(id_pedido);
	...
	free(id_pedido);
    free(pedido_params.nombre.nombre);

	if (!rta_OPcom) {
		log_debug(logger_mensajes, "[MENSJS]: Hubo error al recibir respuesta de comanda -> ERROR");
		responder_ERROR(socket_cliente);
        pthread_mutex_unlock(cliente_en_cuestion->mutex);
		return;
	}
	switch (rta_OPcom->tipo_mensaje) {
        case ERROR:
            log_debug(logger_mensajes, "[MENSJS]: Se recibio ERROR de comanda -> ERROR");
            responder_ERROR(socket_cliente);
        ...
        case RTA_OBTENER_PEDIDO:
            ...
            rta_a_cliente_params->platos = params_rta_OPcom->infoPedidos;
            ...
            loggear_mensaje_enviado(rta_a_cliente_params, RTA_CONSULTAR_PEDIDO, logger_mensajes);
            ...
            return;
        default:
            log_debug(logger_mensajes, "[MENSJS]: La respuesta de comanda no fue la esperada -> ERROR");
            responder_ERROR(socket_cliente);
            free_struct_mensaje(rta_OPcom->parametros, rta_OPcom->tipo_mensaje);
            free(rta_OPcom);
            pthread_mutex_unlock(cliente_en_cuestion->mutex);
...
void gestionar_PLATO_LISTO(m_platoListo* plato_params, int socket_rest) {
	...
	log_debug(logger_mensajes, "[MENSJS]: Llego plato listo (rest %s, pedido %u)",
        plato_params->restaurante.nombre,
        plato_params->idPedido
    );
    loggear_mensaje_recibido(plato_params, PLATO_LISTO, logger_mensajes);

	// mandar plato listo a comanda
	mje_rtaPL = mensajear_comanda(PLATO_LISTO, plato_params, false);
	if (!mje_rtaPL) {
		log_debug(logger_mensajes, "[MENSJS]: Hubo error al recibir rta. comanda -> FAIL a rest.");
		responder_confirm(socket_rest, false, RTA_PLATO_LISTO);
		free_struct_mensaje(plato_params, PLATO_LISTO);
		return;
	}
	switch (mje_rtaPL->tipo_mensaje) {
		case ERROR:
			log_debug(logger_mensajes, "[MENSJS]: Comanda respondio ERROR -> FAIL a rest.");
			responder_confirm(socket_rest, false, RTA_PLATO_LISTO);
			free_struct_mensaje(plato_params, PLATO_LISTO);
		...
		case RTA_PLATO_LISTO:
			if (!*(int*)(mje_rtaPL->parametros)) {
				log_debug(logger_mensajes, "[MENSJS]: Comanda respondio FAIL -> FAIL a rest.");
				responder_confirm(socket_rest, false, RTA_PLATO_LISTO);
				free_struct_mensaje(plato_params, PLATO_LISTO);
				free_struct_mensaje(mje_rtaPL->parametros, mje_rtaPL->tipo_mensaje);
				free(mje_rtaPL);
			...
			if (!mje_rtaOP) {
				log_debug(logger_mensajes, "[MENSJS] Hubo un error al recibir la rta. comanda -> FAIL a rest.");
				responder_confirm(socket_rest, false, RTA_PLATO_LISTO);	
				free_struct_mensaje(plato_params, PLATO_LISTO);
				return;
			}
			switch (mje_rtaOP->tipo_mensaje) {
				case ERROR:
					...
					responder_confirm(socket_rest, false, RTA_PLATO_LISTO);
					free_struct_mensaje(plato_params, PLATO_LISTO);
					free_struct_mensaje(mje_rtaOP->parametros, mje_rtaOP->tipo_mensaje);
					free(mje_rtaOP);
					return;
				case RTA_OBTENER_PEDIDO:
					...
					if (todosLosPlatosEstanPreparados(mje_rtaOP->parametros)) {
						...
						if (!pedido) {
							log_debug(logger_mensajes, "[MENSJS] No se encontro el pedido %i del rest. %s -> FAIL",
									plato_params->idPedido,
									plato_params->restaurante.nombre
							);
							responder_confirm(socket_rest, false, RTA_PLATO_LISTO);
						...
						log_debug(logger_mensajes, "[MENSJS] Se encontro el pedido %i del rest. %s -> OK",
								pedido->pedido_id,
								pedido->restaurante->nombre
						);
						responder_confirm(socket_rest, true, RTA_PLATO_LISTO);
						pthread_mutex_unlock(pedido->estaPreparado);
						// TODO: ok al restaurant
					} else {
						log_debug(logger_mensajes, "[MENSJS] El pedido no esta completo todavia -> OK a rest.");
						responder_confirm(socket_rest, true, RTA_PLATO_LISTO);
					}
					free_struct_mensaje(plato_params, PLATO_LISTO);
					free_struct_mensaje(mje_rtaOP->parametros, mje_rtaOP->tipo_mensaje);
					...
					return;
				default:
					log_debug(logger_mensajes, "[MENSJS]: Comanda no respondio lo esperado -> FAIL a rest.");
					responder_confirm(socket_rest, false, RTA_PLATO_LISTO);
					free_struct_mensaje(plato_params, PLATO_LISTO);
...
t_mensaje* mensajear_comanda(op_code cod_op_env, void* params, bool liberar_params) {
	...
	loggear_mensaje_enviado(mensaje_env->parametros, mensaje_env->tipo_mensaje, logger_mensajes);
	...
	loggear_mensaje_recibido(mensaje_rec->parametros, mensaje_rec->tipo_mensaje, logger_mensajes);
	return mensaje_rec;
}
...
void* qr_admin(t_restaurante* rest) { // pthread_create(rest->q_admin, NULL, qr_admin, rest);
	...
	while (1) {
		...
		if (q_anulada) {
			log_debug(logger_mensajes, "[Q (\"%s\")] Q anulada; seteando flag error...", rest->nombre);
...
// 		index++
// 	) {
// 		cliente = list_get(clientes, index);
// 		pthread_mutex_lock(cliente->mutex);
// 		if (cliente->restaurante_seleccionado == rest) {
// 			cliente->restaurante_seleccionado = NULL;
// 		}
// 		pthread_mutex_unlock(cliente->mutex);
// 	}
